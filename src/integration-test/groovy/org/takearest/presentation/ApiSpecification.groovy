package org.takearest.presentation

import groovy.json.JsonSlurper
import groovy.text.SimpleTemplateEngine
import org.takearest.presentation.transitions.BookingData
...
//
//    when:
//    response = close(request(uri('/api/bookings'), HAL_JSON).
...
//
//    when:
//    def createdBookingPayload = request(bookingURI, HAL_JSON).get(String)
//
//    then:
//    def createdBooking = assertTemplateNotStrict('booking-created.json', HAL_JSON, createdBookingPayload, [
//        bookingURI: bookingURI
...
//
//    when:
//    response = close(request(cancelAction.href as String, HAL_JSON).method(cancelAction.method as String))
//
//    then:
//    204 == response.status
//
//    when:
//    response = close(request(bookingURI, HAL_JSON).get())
//
//    then:
//    404 == response.status
//  }
...
//
//    when:
//    response = request(uri('/api/bookings'), HAL_JSON).
//        post(entity(new CreateBookingTransition(
//            roomId: null,
//            data: null
//        ), APPLICATION_JSON))
